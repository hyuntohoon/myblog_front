...
export function ProfileApp({ reviews, profile }: { reviews: MemberReview[], profile: MemberProfile }) {
  ...
  const [lyrics, setLyrics] = useState<{ trackId: string, progressMs: number | null, progressAtMs: number | null, durationMs: number | null, albumCoverUrl: string | null, live: boolean } | null>(() => {
    if (typeof window === 'undefined')
      return null
    try {
      const id = new URLSearchParams(window.location.search).get('lyrics')
      return id ? { trackId: id, progressMs: null, progressAtMs: null, durationMs: null, albumCoverUrl: null, live: false } : null
    }
    catch {
      return null
    }
  })
  const openLyrics = (t: LyricsOpenTarget) => setLyrics({ trackId: t.trackId, progressMs: t.progressMs, progressAtMs: t.progressAtMs, durationMs: t.durationMs, albumCoverUrl: t.albumCoverUrl, live: true })
  ...
  const openStaticLyrics = (spotifyTrackId: string) => setLyrics({ trackId: spotifyTrackId, progressMs: null, progressAtMs: null, durationMs: null, albumCoverUrl: null, live: false })
  const closeLyrics = () => {
    setLyrics(null)
    try {
      const url = new URL(window.location.href)
...
{profile.handle}
      ...
      {lyrics && <LyricsViewer key={lyrics.trackId} spotifyTrackId={lyrics.trackId} initialProgressMs={lyrics.progressMs} initialProgressAtMs={lyrics.progressAtMs} initialDurationMs={lyrics.durationMs} initialAlbumCoverUrl={lyrics.albumCoverUrl} canRefresh={lyrics.live} onClose={closeLyrics} />}
    </div>
  )
}
